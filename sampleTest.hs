...
describePlaceTemperatures pairs = [(place,des) | (place,temp) <- pairs, let des = describeTemperature temp ]
...
hotter _ [] = []
hotter n ((place, temp):rest)
    | temp > n  = place : hotter n rest
    | otherwise = hotter n rest
...
insert n (Node x xs)
      | n <= x = Node n (Node x xs)
      | otherwise = Node x (insert n xs) 
